/**
 * Discharge Follow-ups billing-sections scan
 *
 * Discharge desk `/discharge?section=follow-ups` scopes the shared follow-up
 ...
 * Financial clearance / pharmacy returns / invoices remain on Billing and
 * other discharge tabs — not duplicated here.
 *
...
describe('discharge Follow-ups billing-sections scan (IPD)', () => {
  const scheduledFollowUp = {
    id: 'fu-uuid-ipd-1',
    human_friendly_id: 'FU-IPD-1',
  ...
  it('IPD Follow-ups list read does not touch patient billing ledger', async () => {
    ...
    expect(result.followUps[0].id).toBe('FU-IPD-1');
  ...
  it('IPD reschedule stays NOT_BILLED', async () => {
    ...
    followUpRepository.update.mockResolvedValue({
      id: 'fu-uuid-ipd-1',
      status: 'SCHEDULED',
      notes: 'Rescheduled post-discharge call',
    ...
    expect(result.notes).toBe('Rescheduled post-discharge call');
  ...
  it('IPD complete replay does not duplicate billing posts', async () => {
    ...
    await completeFollowUp('FU-IPD-1', {}, 'user-123', '127.0.0.1');
    followUpRepository.findById.mockResolvedValue({
      id: 'fu-uuid-ipd-1',
      status: 'COMPLETED',
    });
